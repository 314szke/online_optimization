...
\section{Online \emph{convex} covering with multiple experts}

While linear programs are suitable to represent many optimization problems, they are too simple to capture some practically relevant problems (for example, makespan minimization and congestion management). We take a step further towards a general framework, and propose an algorithm with multiple exerts for problems with convex objectives and linear constraints.
