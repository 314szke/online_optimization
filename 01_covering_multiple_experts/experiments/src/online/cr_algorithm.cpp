...
CR_Algorithm::CR_Algorithm(const OfflineModel& model, const Config& config, const Experts& experts) :
    _model(model),
    _experts(experts),
    internal_model(config, model, experts),
    lp_solver(internal_model, config.gurobi_verbosity),
    frank_wolfe(config, internal_model, lp_solver),
...
const DoubleVec_t& CR_Algorithm::solve()
{
    ...
    for (uint32_t t = 1; t < (_model.getNbConstraintBatches() + 1); t++) {
        internal_model.revealNextConstraints();
        lp_solver.addNewConstraints(t);

        DoubleVec_t w = frank_wolfe.solve(internal_model.getInitialSolution());
        verifyWeights(w);
        ...
        Solution::RoundSolutionIfNeeded(_model, sub_solutions[(t-1)], internal_model.getNbOriginalRevealedConstraints());
...
{
    for (uint32_t v = 0; v < internal_model.getNbVariables(); v++) {
        if (w[v] < 0) {
            std::stringstream message;
