...
During our algorithm's description and analysis, we use the following two notions of function smoothness. We assume that the original covering problem's objective function $f$ is convex, non-decreasing (meaning $\nabla_i f(x^t) > \nabla_i f(x^{t-1})$ if $x_i^t$ > $x_i^{t-1}$), $L$-gradient-Lipschitz, and $(\lambda, \mu)$-locally-smooth.
