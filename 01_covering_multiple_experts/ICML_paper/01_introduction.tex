...
The non-linear problem setting is analogous to the linear one, since we consider non-linear problems with linear constraints. The only difference compared to the previous paragraph is the total cost of the solution $(x_{i}^t)_{i=1}^{n}$, which becomes $f(x^t)$, where $f$ is a non-linear function.

\noindent \textbf{Experts.} \label{subsec:experts} In our model, the experts' predictions are also online solutions. In other words, the experts' solutions
...
\paragraph{Approach.} We use the primal-dual approach to design competitive algorithms with the new \texttt{LIN-COMB} benchmark. First, we relax the linear program formulation of \texttt{LIN-COMB}, which serves as a lower bound. Then, we take the dual of the relaxation, which is a lower bound on the relaxation. Therefore, following the chain of lower bounds, the dual problem is a lower bound on the \texttt{LIN-COMB} benchmark. The formulations of the relaxation and its dual are detailed in \cref{sec:covering} for linear problems and in \cref{sec:convex} for non-linear problems.
...
In the non-linear setting, there is no obvious choice for the regularization function.
In our approach, we propose a regularizer by coupling the entropy-like function with the gradient-Lipschitz property of the original covering problem's objective function.
...
First, we present an algorithm for the online linear covering problem that achieves an objective cost at most $O(\ln(K\rho))$ times the cost of the \texttt{LIN-COMB} benchmark. Building on the salient ideas of the linear case, we extend our approach for the online non-linear covering problem.
...
\begin{figure}[!ht]
	...
	%	\begin{spacing}{0.1}
		...
		%\vspace{5pt}
	\end{mdframed}
	\caption{Formulation of the \texttt{LIN-COMB} benchmark for both the linear and non-linear problems with linear covering-type constraints}
	\label{fig:benchmark}
\end{figure}
...
In this paper we show two algorithms to solve online \emph{linear} and online \emph{non-linear} covering problems with multiple experts. While the non-linear setting includes the linear one, our proposed algorithm is simpler for the linear case, so easier to understand. \cref{sec:covering} details our first algorithm for the linear case, and due to the page limitations, we detail in \cref{sec:convex} the extension to the non-linear setting. \cref{sec:exp} shows empirical results, and we conclude in \cref{sec:conclusion}. \cref{appix-tight-solutions} and \cref{sec:counter-example} further details some
related work discussions. %and \cref{appix-applications} shows some applications for the non-linear setting.
