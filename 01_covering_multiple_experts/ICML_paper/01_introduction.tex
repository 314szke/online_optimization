...
Both of our proposed algorithms set the decision variables at every time step based on the solution of an internal convex program. 
...
When the objective cost is non-linear function, it was not clear which regularization function should be used. 
In our approach, we propose a regularizer by coupling the entropy-like function and the gradient-Lipschitz property of the objective. 
...
First, we present an algorithm for the online linear covering problem that achieves the objective cost at most $O(\ln(K\rho))$ times the cost of the \texttt{LIN-COMB} benchmark. Subsequently, building on the salient ideas of the linear objective, we extend our approach for the online non-linear objective covering problem. 
We provide an algorithm that achieves an objective cost at most $O(\ln(K\rho)) \cdot \frac{\lambda}{(1-\mu\ln(K\rho))}$ times the cost of \texttt{LIN-COMB}, where $\lambda$ and $\mu$ are the ($\lambda$,$\mu$) are some parameters of the objective function (precisely, ($\lambda$,$\mu$) are smoothness parameters of the function that are defined later in the corresponding section).
...
In this paper we show two algorithms to solve online \emph{linear} and online \emph{convex} covering problems with multiple experts. While the convex setting includes the linear one, our proposed algorithm is simpler for the linear case, so easier to understand. \cref{sec:covering} details our first algorithm for the linear case, and due to the page limitations, we detail in \cref{sec:convex} the extension to the convex setting. \cref{sec:exp} shows empirical results, and we conclude in \cref{sec:conclusion}. \cref{appix-tight-solutions} and \cref{sec:counter-example} further details some
related work discussions, and \cref{appix-applications} shows some applications for the convex setting.
