...
The relaxed formulation of \texttt{LIN-COMB} and its dual are below.  

\begin{minipage}{0.45\textwidth}
		\begin{align*}
			&& \min \sum_{t = 1}^{T} \sum_{i=1}^{n} & c_i y_i^t \\
			%
			(\alpha^{t}) \quad && \sum_{k=1}^{K} w_{k}^{t} & \geq 1  & \forall\ t \\
			%
			(\beta_{i}^{t}) \quad && \sum_{k=1}^{K} \left(w_{k}^{t} s_{i,k}^{t} - w_{k}^{t-1} s_{i,k}^{t-1} \right) &\leq y_i^t  &\forall\ i,t\\
		%
...
After down-scaling the experts' suggestions, do the following for each expert $k$. 
\begin{compactenum}
...
%
	\begin{wrapfigure}{r}{0.5\textwidth}
		\vspace{-1.1cm}
		\centering
		\includegraphics[width=0.4\textwidth]{./Img/algo_structure.pdf}
		\vspace{-0.35cm}
	...
	serves as an illustration in which  $E_1,\ E_2, \dots\ E_K$ correspond to the experts of the online problem.)
